...
export function titleFor(context, workspace, stage, sessionId, knownSession = null) {
  ...
  return renderTitle({ workspace, stage, ordinal: current.ordinal, summary: current.summary, state: current.state, trackerLinks: stageMeta.tracker_links?.length ? stageMeta.tracker_links : meta.tracker_links, issuePrefix: context.storage.config.title?.issue_prefix || "short" });
}
