...
export const work_context_update_stage_prompt = tool({ description: "Update the optional working prompt of an existing stage.", args: { workspace: tool.schema.string().regex(/^\d{6}$/).optional(), stage: tool.schema.string().regex(/^\d{1,2}$/), prompt: tool.schema.string() }, execute: run((ctx, a) => ctx.updateStagePrompt(workspaceFromSession(ctx, a.workspace), a.stage, a.prompt)) });
